...
export class ProviderService {
  ...
  async createProvider(data: { name: string; slug: string }): Promise<Provider> {
    const provider = this.providerRepository.create(data);
    return await this.providerRepository.save(provider);
  }

  async updateProvider(id: number, data: { name?: string; slug?: string }): Promise<Provider | null> {
  ...
  async getProviderStats(id: number): Promise<any> {
    const [
      userCount,
      bannerCount,
      categoryCount,
  ...
  async getProviderByDomain(domain: string): Promise<Provider | null> {
  ...
  async createProviderWithDefaultData(data: { name: string; slug: string }): Promise<Provider> {
