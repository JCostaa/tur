...
import { ArrowBack, Star, LocationOn, Group, Wifi, Pool, LocalParking } from '@mui/icons-material';
...
const AccommodationDetail: React.FC = () => {
  ...
  // Mock de dados da hospedagem
  const hotel = {
    title: 'Pousada Pantanal Encantado',
    location: 'Pantanal',
    rating: 4.8,
    price: 'R$420,00/noite',
    people: 2,
    description:
      'Desfrute de conforto e contato com a natureza na Pousada Pantanal Encantado. Quartos aconchegantes, café da manhã incluso, piscina, Wi-Fi e estacionamento gratuito. Ideal para famílias e casais.',
    image: '/images/category-1.jpg',
    amenities: ['Wi-Fi grátis', 'Piscina', 'Estacionamento', 'Café da manhã incluso', 'Ar-condicionado'],
  };
    ...
    <Box sx={{ background: '#f8f9fa', minHeight: '100vh', pb: 8 }}>
      ...
      <Container maxWidth="lg">
        <InfoCard>
          ...
          <Subtitle>
            <LocationOn sx={{ mr: 1, fontSize: 22 }} /> {hotel.location}
          </Subtitle>
          <ChipsRow>
            <Chip icon={<Star sx={{ color: '#FFD700' }} />} label={`${hotel.rating} estrelas`} />
            <Chip icon={<Group />} label={`${hotel.people} pessoas`} />
            <Chip icon={<Wifi />} label="Wi-Fi" />
            <Chip icon={<Pool />} label="Piscina" />
            <Chip icon={<LocalParking />} label="Estacionamento" />
          </ChipsRow>
