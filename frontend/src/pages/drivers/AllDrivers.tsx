...
const AllDrivers: React.FC = () => {
  ...
  const { data: driversData, isLoading, isError } = useQuery({
    queryKey: ['drivers'],
    queryFn: getDrivers,
  });

  // Ignorar filtros e listar todos os motoristas
  const allDrivers = Array.isArray(driversData?.data?.providers) ? driversData.data.providers : [];
  ...
  const mapDriverToPackage = (driver: any) => ({
    ...
    location: driver.location?.city || driver.location?.address || 'Local não informado',
    ...
    description: driver.content ? driver.content.replace(/<[^>]+>/g, '') : '', // remove HTML
  });
  const mappedPackages = allDrivers.map(mapDriverToPackage);

  // Gerar lista dinâmica de cidades a partir dos dados
  const dynamicLocations: string[] = Array.from(new Set(
    (driversData?.data?.providers || [])
      .map((driver: any) => driver.location?.city)
      ...
      <Box sx={{ padding: isMobile ? '24px 0' : '40px 0', minHeight: '100vh', background: theme.palette.background.default, marginTop: 10 }}>
        <Container maxWidth="xl">
          ...
          <Box sx={{ display: { xs: 'block', md: 'flex' }, gap: 4 }}>
            ...
            <Box sx={{ flex: 1 }}>
              {isLoading ? (
                <Typography align="center" sx={{ mt: 8, color: '#888' }}>Carregando condutores...</Typography>
              ) : isError ? (
                <Typography align="center" sx={{ mt: 8, color: 'error.main' }}>Erro ao carregar condutores.</Typography>
              ) : allDrivers.length === 0 ? (
                <Typography align="center" sx={{ mt: 8, color: '#888' }}>Nenhum condutor encontrado.</Typography>
              ) : (
                <TravelPackages customPackages={mappedPackages} hideTitle showArrows={false} onCardClick={handleDriverCardClick} />
              )}
            </Box>
          </Box>
        </Container>
