            ...
            <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 3, mb: 4 }}>
              ...
              <Box sx={{ textAlign: 'right' }}>
                ...
                <ContactButton variant="contained" color="primary" startIcon={<Phone />} onClick={() => {
                  const phone = driver?.provider?.phone_number || driver.phone
           
                  if (phone) {
                    // Remove any non-numeric characters except +
                    const cleanPhone = phone.replace(/[^\d+]/g, '');
                    // Create WhatsApp URL with pre-filled message
                    const message = encodeURIComponent(`Olá! Tenho interesse nos serviços de condução "${driver.title || driver.name}". Gostaria de mais informações.`);
                    const whatsappUrl = `https://wa.me/${cleanPhone}?text=${message}`;
                    window.open(whatsappUrl, '_blank');
                  } else {
                    alert('Número de telefone não disponível para este condutor.');
                  }
                }}>
                  Contatar
                </ContactButton>
