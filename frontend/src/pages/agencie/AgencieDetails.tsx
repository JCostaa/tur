...
const AgencyDetails: React.FC = () => {
  ...
  if (!agency) {
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', gap: 2 }}>
        <Typography color="error" variant="h6">Agência não encontrada.</Typography>
        <Typography color="text.secondary" variant="body2">
          Os dados da agência não foram fornecidos na navegação.
        </Typography>
        <Button 
          variant="contained" 
          onClick={() => navigate('/all-agencies')}
          sx={{ mt: 2 }}
        >
  ...
  const handleNextImage = () => {
    setCurrentImageIndex((prev) => (prev + 1) % (agency.gallery?.length || 1));
  };

  const handlePrevImage = () => {
    setCurrentImageIndex((prev) => (prev - 1 + (agency.gallery?.length || 1)) % (agency.gallery?.length || 1));
  };

  console.log(agency);

  const images = agency.gallery && agency.gallery.length > 0 ? agency.gallery : [agency.image];
        ...
        <ProfileCard>
          <CardContent sx={{ p: 4 }}>
            <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 3, mb: 4 }}>
              <ProfileAvatar src={agency.image} alt={agency.title || agency.name}>
              ...
              <Box sx={{ flex: 1 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                  <Chip 
                    icon={<Star />} 
                    label={`${agency.rating || 5.0} estrelas`} 
                    color="primary" 
                    variant="outlined" 
                    size="small"
                  />
          ...
          <InfoSection>
            ...
            <DataRow>
              ...
              <DataValue sx={{ color: '#FF5722', fontWeight: 700 }}>
                <a href={agency.social_media.instagram} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'underline' , color: '#ff6b35' }}>
                  Clique aqui
                </a>
              </DataValue>
            </DataRow>
