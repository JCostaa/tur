...
const EventDetail: React.FC = () => {
  ...
  const event = React.useMemo(() => {
    const allEvents = Array.isArray(eventsData?.data?.events) ? eventsData.data.events : [];
    return allEvents.find((e: any) => String(e.id) === String(id));
  }, [eventsData, id]);
  ...
  if (isLoading) {
    return <Typography align="center" sx={{ mt: 8 }}>Carregando evento...</Typography>;
  }
  if (isError || !event) {
    return <Typography align="center" sx={{ mt: 8, color: 'error.main' }}>Evento não encontrado.</Typography>;
  }

  // Mapeamento dos campos para exibição
  const highlights = [
    ...(event.attributes?.flatMap((attr: any) => attr.items) || []),
    ...(event.category ? [event.category] : []),
  ];
  const descriptionHtml = event.content || '';
  const location = event.location?.city || event.location?.address || 'Local não informado';
  const duration = event.duration_description || (event.duration ? `${event.duration} min` : 'Duração não informada');
  const price = event.price;
  const image = event.image;
  const rating = 5;
  const people = 2;
  const gallery = Array.isArray(event.gallery) ? event.gallery.filter((img: any) => img && img.large) : [];
  const banner = event.banner || image;
  const howItWorks = event.how_it_works;
  const essentialInfo = event.essential_information;
  const insuranceInfo = event.insurance_info;
  const include = event.include;
  const exclude = event.exclude;
  const info = event.info || {};
  const provider = event.provider || {};
  const video = event.video;
  const faq = Array.isArray(event.faq) ? event.faq : [];
  ...
  return (
    <Box sx={{ background: '#f8f9fa', minHeight: '100vh', pb: 8 }}>
      ...
      <Container maxWidth="lg">
        <InfoCard>
          <Title>{decodeHtmlEntities(event.title)}</Title>
          <Subtitle>
